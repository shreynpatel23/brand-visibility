...
import { Plus, Filter, TrendingUp, TrendingDown, Minus, Building2, BarChart3, Activity, Users, Target, Calendar, ChevronDown } from 'lucide-react';
...
const DashboardPage: React.FC = () => {
  ...
  const brands: Brand[] = [
    {
      ...
      metrics: { totalPrompts: 127, avgResponseTime: 1.2, successRate: 94, lastUpdated: '2 hours ago' },
      weeklyData: {
    ...
    },
    {
      ...
      metrics: { totalPrompts: 89, avgResponseTime: 1.8, successRate: 91, lastUpdated: '4 hours ago' },
      weeklyData: {
        labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
    ...
    },
    {
      ...
      metrics: { totalPrompts: 156, avgResponseTime: 1.5, successRate: 96, lastUpdated: '1 hour ago' },
      weeklyData: {
        labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
  ...
  const MetricCard: React.FC<{ title: string; value: string | number; subtitle?: string; icon: React.ReactNode; trend?: { value: number; isPositive: boolean } }> = ({ title, value, subtitle, icon, trend }) => (
  ...
  const FunnelChart: React.FC<{ scores: Brand['scores'] }> = ({ scores }) => {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-6">Funnel Performance</h3>
        <div className="space-y-4">
          {stages.map((stage, index) => (
            <div key={stage} className="flex items-center">
              <div className="w-16 text-sm font-medium text-gray-600 dark:text-gray-400">{stage}</div>
              <div className="flex-1 mx-4">
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3">
                  <div 
                    className={`h-3 rounded-full ${getScoreColor(scores[stage])}`}
                    style={{ width: `${scores[stage]}%` }}
                  />
                </div>
              </div>
              <div className={`w-12 text-right text-sm font-semibold ${getScoreTextColor(scores[stage])}`}>
                {scores[stage]}%
              </div>
            </div>
          ))}
  ...
  const SentimentAnalysis: React.FC<{ sentiment: Brand['sentiment'] }> = ({ sentiment }) => (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Sentiment Analysis</h3>
        <div className={`flex items-center space-x-1 ${getTrendColor(sentiment.trend)}`}>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div className="flex items-center">
            ...
            <span className="text-gray-600 dark:text-gray-400">Strongly Positive: {sentiment.distribution.stronglyPositive}%</span>
          </div>
          <div className="flex items-center">
            <div className="w-3 h-3 bg-green-400 rounded-full mr-2" />
            <span className="text-gray-600 dark:text-gray-400">Positive: {sentiment.distribution.positive}%</span>
          </div>
          <div className="flex items-center">
            <div className="w-3 h-3 bg-gray-400 rounded-full mr-2" />
            <span className="text-gray-600 dark:text-gray-400">Neutral: {sentiment.distribution.neutral}%</span>
          </div>
          <div className="flex items-center">
            <div className="w-3 h-3 bg-red-400 rounded-full mr-2" />
            <span className="text-gray-600 dark:text-gray-400">Negative: {sentiment.distribution.negative}%</span>
          </div>
        </div>
      </div>
    </div>
  ...
  const ModelPerformance: React.FC<{ performance: Brand['modelPerformance'] }> = ({ performance }) => (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
      <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-6">AI Model Performance</h3>
      <div className="space-y-4">
          ...
          <div key={model} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
            <div className="flex items-center">
              ...
              <div>
                ...
                <div className="text-sm text-gray-500 dark:text-gray-400">{data.prompts} prompts</div>
              </div>
            </div>
            <div className={`text-lg font-semibold ${getScoreTextColor(data.score)}`}>
              {data.score}%
            </div>
          </div>
        ))}
  ...
  const WeeklyTrend: React.FC<{ data: Brand['weeklyData'] }> = ({ data }) => (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
      <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-6">Weekly Performance Trend</h3>
      <div className="space-y-4">
        ...
        {data.labels.map((label, index) => (
          <div key={label} className="flex items-center justify-between">
            <div className="w-12 text-sm font-medium text-gray-600 dark:text-gray-400">{label}</div>
            <div className="flex-1 mx-4 flex items-center">
              <div className="w-1/2 pr-2">
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div 
                    className={`h-2 rounded-full ${getScoreColor(data.scores[index])}`}
              ...
              <div className="w-1/2 pl-2">
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${(data.prompts[index] / Math.max(...data.prompts)) * 100}%` }}
                  />
                </div>
              </div>
            </div>
            <div className="flex space-x-4 text-sm">
              <span className={getScoreTextColor(data.scores[index])}>{data.scores[index]}%</span>
              <span className="text-blue-600 dark:text-blue-400">{data.prompts[index]}</span>
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Brand Analytics</h1>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{selectedBrand.name}</h2>
          ...
          <div className="flex space-x-2">
            <Link
              to={`/app/brands/${selectedBrand.id}/edit`}
              className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
            >
              Edit Brand
            </Link>
            <Link
              to={`/app/prompt-logs?brand=${selectedBrand.id}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <MetricCard
          title="Overall Score"
          value={`${Math.round((selectedBrand.scores.TOFU + selectedBrand.scores.MOFU + selectedBrand.scores.BOFU + selectedBrand.scores.EVFU) / 4)}%`}
          icon={<TrendingUp className="w-5 h-5 text-primary" />}
          trend={{ value: selectedBrand.sentiment.percentage, isPositive: selectedBrand.sentiment.trend === 'up' }}
        />
      </div>
